...
let timing_test _ =
  ...
  let rope = List.fold_left (fun acc gj ->
    match acc with
    | Some(a) ->
        Some(Limitless.Rope.concat a (Leaf [gj]))
    | None ->
        Some(Leaf [gj])) None all_j |> Option.get in ();
  let end' = Unix.gettimeofday () -. start in
  Printf.printf "timing_test took: %f. Rope length: %d\n" end' (Limitless.Rope.length rope);
...
let tests =
  ...
  >::: [
         ...
         "rope concatenation time" >:: timing_test;
       ]
