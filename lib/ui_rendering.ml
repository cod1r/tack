...
let write_container_values_to_ui_buffer ~(box : Ui.box) ~(buffer : render_buffer_wrapper) =
  let Ui.{ width; height; x; y; _ } = Option.value box.bbox ~default:Ui.default_bbox
...
let write_text_to_ui_buffer
  ...
  Float.Array.iteri (fun idx v -> render_buf_container.buffer.{idx + start} <- v) values;
...
let write_highlight_to_ui_buffer
...
let write_cursor_to_ui_buffer
      ~(render_buf_container : render_buffer_wrapper)
      ~x
      ~y
      ~font_height
  =
  let points = [ x, y + font_height; x, y; x + 3, y; x + 3, y + font_height ] in
    ...
    let fold_fn_draw_cursor acc c =
      ...
      then
        write_cursor_to_ui_buffer
          ~render_buf_container:ui_buffer
          ~x:acc.x
          ~y:acc.y
...
let clip_content ~(box : Ui.box) =
  ...
  try
    ...
    Opengl.gl_scissor
      ...
      ~height:bbox.height
  with
  | Invalid_argument e -> failwith ("clipping needs a bbox;" ^ e)
;;
...
let handle_list_of_boxes_initial_position
      ~(box : Ui.box)
      ~(d : Ui.direction)
      ~(list : Ui.box list)
  =
...
let draw_textarea
  ...
  match rope with
  | Some r ->
    draw_text_textarea ~font_info ~rope:r ~bbox ~scroll_y_offset ~text_buffer:ui_buffer;
    draw_to_gl_buffer_text ();
