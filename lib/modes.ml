        ...
        | Some r -> (
            ...
            | _ -> (
                ...
                | '\t' when kbd_evt_type = Keydown ->
                    ...
                    in
                    let new_editor : Editor.editor =
                      { editor with ropes = new_rope_wrapper :: new_ropes }
                    in
                    Render.draw new_editor;
                    new_editor
                | _ -> editor))
...
(*                         (* backspace *) *)
(*                         let rope_len = Rope.length r in *)
(*                         if rope_len > 0 && editor.cursor_pos > 0 then ( *)
(*                           let new_rope = *)
(*                             Some (Rope.delete r (editor.cursor_pos - 1) 1) *)
(*                           in *)
(*                           let new_editor : Editor.editor = *)
(*                             { *)
(*                               editor with *)
(*                               rope = new_rope; *)
(*                               cursor_pos = editor.cursor_pos - 1; *)
(*                             } *)
(*                           in *)
(*                           Render.draw new_editor; *)
(*                           new_editor) *)
(*                         else editor *)
(*                     | 'c' when kbd_evt_type = Keydown -> *)
(*                         (match (editor.search_rope, editor.highlight) with *)
(*                         | Some r, Some (start, end') -> *)
(*                             Rope.substring r ~start ~len:(end' - start) *)
(*                             |> Rope.to_string |> Sdl.set_clipboard_text *)
(*                         | _ -> ()); *)
...
(*                               match editor.search_rope with *)
(*                               | Some r -> *)
(*                                   Rope.insert r editor.cursor_pos *)
(*                                     clipboard_contents *)
...
(*                         (* horizontal tab will be two spaces *) *)
(*                         let new_rope = *)
(*                           Some (Rope.insert r editor.cursor_pos "  ") *)
(*                         in *)
(*                         let new_editor : Editor.editor = *)
(*                           { *)
(*                             editor with *)
(*                             rope = new_rope; *)
(*                             cursor_pos = editor.cursor_pos + 2; *)
(*                           } *)
(*                         in *)
(*                         Render.draw new_editor; *)
(*                         new_editor *)
...
(*               button clicks timestamp; *)
(*             let crp = *)
(*               if Option.is_some editor.search_rope then *)
...
(*     | Some (TextInputEvt { text; _ }) -> *)
(*         let new_rope = *)
(*           match editor.Editor.search_rope with *)
