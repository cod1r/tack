...
CAMLprim value init_sdl(value unit) {
  ...
  int res_setting_core_profile = SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_CORE);
  if (res_setting_core_profile) {
    result = caml_alloc(1, 1);
    Store_field(result, 0, caml_copy_string(SDL_GetError()));
    CAMLreturn(result);
  }
  int res_setting_major_version = SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, 4);
  if (res_setting_major_version) {
    result = caml_alloc(1, 1);
    Store_field(result, 0, caml_copy_string(SDL_GetError()));
    CAMLreturn(result);
  }
  int res_setting_minor_version = SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, 1);
  if (res_setting_minor_version) {
    result = caml_alloc(1, 1);
    Store_field(result, 0, caml_copy_string(SDL_GetError()));
    CAMLreturn(result);
  }
