...
void push_to_buffer(struct Buffer* b, const struct GlyphInfo glyph_info, int window_width, int window_height, int x_offset, int y_ppem_c) {
  ...
  int row = (x_offset + x_advance) / window_width;
  float y_ppem_c_norm = y_ppem_c / (float)window_height * 13;
  float y_offset = row * y_ppem_c_norm;

  float x_offset_adjusted = ((x_offset + (int)x_advance) % window_width) / (float)window_width;
  ...
  for (int buffer_idx = b->size; buffer_idx < b->size + glyph_info.buffer.size; buffer_idx += 3) {
    ...
    float altered_y = -glyph_info.buffer.contents[second - b->size] / window_height + 1.0 - y_ppem_c_norm + horiBearingY;
...
*/
CAMLprim value write_to_buffer(value buffer, value glyph_info, value window_width, value window_height, value previous_offset, value y_ppem) {
  CAMLparam5(buffer, glyph_info, window_width, window_height, previous_offset);
  CAMLxparam1(y_ppem);

  int x_offset = Int_val(previous_offset);

  int y_ppem_c = Int_val(y_ppem);

  struct GlyphInfo* glyph_info_struct = *(struct GlyphInfo**)Data_abstract_val(glyph_info);

  struct Buffer* b = *(struct Buffer**)Data_abstract_val(buffer);

  push_to_buffer(b, *glyph_info_struct, Int_val(window_width), Int_val(window_height), x_offset, y_ppem_c);
