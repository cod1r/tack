...
let find_closest_vertical_range ~(box : box) ~(font_info : Freetype.font_info)
  ...
  let _, Rope_types.{closest_vertical_range; _} =
    Rope.traverse_rope ~box ~font_info ~rope ~handle_result:None
      ~result:
...
let find_closest_rope_pos_for_cursor_on_coords ~(box : box)
    ~(font_info : Freetype.font_info) ~x ~y ~rope =
  ...
  let closest_vertical_range =
    find_closest_vertical_range ~box ~font_info ~rope ~y
  in
  let closest_rope =
...
let find_coords_for_cursor_pos ~(box : box) ~font_info ~rope ~cursor_pos =
  ...
  | Some acc ->
      acc
  | None ->
      "EXPECTED TO HAVE SOME COORDS " ^ __LOC__ |> failwith

let find_closest_rope_pos_for_moving_cursor_in_vertical_range
    ~(font_info : Freetype.font_info) ~cursor_x ~lower_y ~rope ~box =
  let hor_pos =
    find_closest_horizontal_pos ~font_info ~rope ~x:cursor_x ~box
